...
class Concat(nn.Module):
    # Concatenate a list of tensors along dimension沿指定维度拼接多个张量
...
def autopad(k, p=None, d=1):  # kernel, padding, dilation
    # Pad to 'same' shape outputs用于根据给定的内核大小、填充和膨胀值计算填充大小，以保持输出形状不变。
    if d > 1:
...
class C3(nn.Module):
    # CSP Bottleneck with 3 convolutions一个具有3个卷积层的 CSP（Cross Stage Partial）残差块
...
class SPPF(nn.Module):
    # Spatial Pyramid Pooling - Fast (SPPF) layer for YOLOv5 by Glenn Jocher一个空间金字塔池化（Spatial Pyramid Pooling）层。
