...
if __name__ == '__main__':
    ...
    # 这段代码的功能是使用YOLOv5模型对输入图像进行目标检测，并在图像上绘制检测到的目标框。首先，定义了辅助函数用于坐标转换和计算交并比。
    # 然后，创建了YOLOv5模型并加载预训练权重。接下来，读取输入图像并进行预处理，将其转换为Tensor格式。然后，运行模型进行推理，得到预测结果。
    # 最后，对预测结果进行非极大值抑制，筛选出重叠较少且置信度较高的检测框，并在图像上绘制这些框。